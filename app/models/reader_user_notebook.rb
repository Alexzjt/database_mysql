class ReaderUserNotebook < ActiveRecord::Base
  def self.sort_list
    ...
    notebook_list_array =Oj.load(notebook_obj.list)
    ...
    array = notebook_list_array.keys
    array.sort! do |a, b|
      begin
        time_a = Time.mktime(notebook_list_array[a])
        time_b = Time.mktime(notebook_list_array[b])
